...
// REST API
@Slf4j // 로그 찍어주는 어노테이션
@RestController // 데이터만 요청하고 데이터만 응답 받는 Controller
public class BasicController {
     ...
     *         -> 공백이나 NULL일 때 기존의 데이터를 수정 하지 않는 거임.
     ...
     *      상품 여러개(다건) 조회(GET) /products(전체 조회), /products?page=1&count=30(한페이지에 30개씩)
     *      상품 수정(PUT) /product/1(id, key)
     *      상품 삭제(DELETE) /product/1(id, key)
     *      (주소는 같지만 요청 메소드는 다 다르다.)
     *
    ...
    public Object teacherPost(@RequestBody ReqTeacherDto reqTeacherDto) {
